node {
  ...
  stage('Deploy to Swarm Cluster') {
    ...
    if(isPresent?.trim().equals('emp_dir')) {
      sh "ssh -t root@swarm-master.tmsapp.us 'docker service update --replicas 10 --update-delay 2s --update-parallelism 2 --image sameetn/emp_dir:${env.BUILD_NUMBER} emp_dir'"  
    } else {
      sh "ssh -t root@swarm-master.tmsapp.us 'docker service create --replicas 10 --update-delay 2s --update-parallelism 2 --name emp_dir sameetn/emp_dir:${env.BUILD_NUMBER}'"
    }
  }
}
